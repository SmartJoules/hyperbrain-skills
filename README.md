...
6. **Engineering Standards** - Mandatory OOP+SOLID, design patterns (strategy/factory/builder/observer/decorator), DRY/KISS, minimal diffs, resilience (no leaks/unhandled promises, error/loading/empty/partial states), query optimization, caching with eviction, and Kafka/Redis connection standards (heartbeat, offset, lag, singleton, retry)
   ...
   - **Inference Engineering** - Production AI inference engineering for LLM/ML/RAG/agent serving: model routing, prompt/model versioning, structured output validation, streaming, batching, caching, retries, rate limits, cost/latency SLOs, observability, eval gates, fallbacks, and safe DeJoule/Lumen inference paths.
   - **Data Modeling Algorithm** - Access-pattern-first SQL/NoSQL data modeling for PostgreSQL/MySQL, DynamoDB, MongoDB, time-series, Redis, and graph stores: entities, relationships, constraints, tenant/site scope, indexes, composite keys, GSIs/LSIs, projections, migrations, backfills, and verification.
   ...
   - **DeJoule Coding** - Repo-grounded style guide for writing JouleTRACK and jt-api-v2 code in DeJoule/SmartJoules production style: narrow Sentry-driven fixes, PR feedback cleanup, Angular/NgRx and Sails/Waterline patterns, LLD/system design expectations, focused tests, and PR hygiene.
   ...
   - **Architecture Reviewer** - Principal Engineer / Solution Architect audit of an EXISTING system's architecture (not line-level, not greenfield): boundaries, coupling/cohesion, layering, dependency direction, scalability, data flow, resilience, security posture, observability, and architectural smells. Findings by severity + remediation roadmap + health verdict. Complements code-reviewer (code grain) and software-architecture-planner (forward design).
   - **Design Doc Reviewer** - Gates design docs (HLD/LLD, RFC, spec, ADR) against required structure, engineering constraints, and the applicable hyperbrain skills — APPROVE / APPROVE-WITH-CHANGES / REJECT with actionable fixes. Ships a deterministic CI linter (`scripts/design-doc-lint.js`, no API key needed) + a GitHub `workflow-template.yml` (blocking gate + advisory Claude review) + `CONSTRAINTS.md`, so a non-conforming doc can fail the PR check.
   - **Design Doc Harness** - The end-to-end design-doc process used by top product companies: a fill-in FAANG-style template (`templates/DESIGN_DOC_TEMPLATE.md` + `ADR_TEMPLATE.md`), the lifecycle (Draft → human review → human approval → automated review → implementation), roles (author/reviewers/approver), doc tiers (ADR vs RFC vs architecture doc), and how it plugs into the CI gate + review agent. The authoring side that pairs with the reviewer.
   - **Design Doc Review Agent** - A *callable* Pi-style reviewer agent (`/skill:design-doc-review-agent <doc>`; also works in Claude Code) that applies the design-doc-reviewer rules and grounds every piece of feedback in the real context + coding standards of the target repo — JouleTRACK (Angular), jt-api-v2 (Sails), iot-feedback-handler (TypeScript microservice) — plus hyperbrain engineering-standards. Posts line-anchored PR feedback; composes design-doc-reviewer + pi-coding-agent (doesn't duplicate them).
   ...
   - **iot-feedback-handler-knowledge-base** - KB for the iot-feedback-handler event-driven TypeScript microservice: layered architecture (handlers→orchestrators→services→repositories), mandated Repository/Factory/Strategy/Singleton patterns, KafkaJS+snappy consumer (manual offset, no DLQ), cloud/edge DocumentStoreFactory (DynamoDB/MongoDB) + Postgres audit + Redis cache, coding standards, data flow, and the mandated Morpheus MCP usage.
