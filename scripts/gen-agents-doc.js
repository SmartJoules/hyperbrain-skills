...
lines.push('');
lines.push('- **Claude Code** — name the skill in your ask ("use the *code-reviewer* skill on this diff") or via the Skill tool. Skills install under `~/.claude/skills/` (`./install.sh`).');
lines.push('- **Pi (pi.dev)** — `/skill:<name>` (e.g. `/skill:design-doc-review-agent <doc>`); args after the command are passed to the skill. Install via `./install.sh --assistant pi` → `~/.pi/agent/skills/`.');
